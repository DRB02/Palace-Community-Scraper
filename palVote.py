...
def main():
    ...
    for card in cards:
        ...
        scoretab = card.find('div',{'style':'display:flex;flex-direction:row'})
        score = scoretab.find_all('div')
        upvote = score[0].text
        downvote = score[1].text
        ...
        embed.add_embed_field(name='Upvotes:', value=upvote,inline=True)
        embed.add_embed_field(name='Downvotes:', value=downvote,inline=True)
